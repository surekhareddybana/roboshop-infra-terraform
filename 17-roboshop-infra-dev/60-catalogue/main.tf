resource "aws_lb_target_group" "catalogue" {
  ...
  health_check {
    healthy_threshold = 2
    interval = 5 #healthcheck intrevel means every 5 secs health check chestundi
    matcher = "200-299"
    path = "/health"
    port = 8080
    timeout = 2 # 2 sec lopu respponse raavali
...
resource "terraform_data" "catalogue_delete" {
  ...
  provisioner "local-exec" { #manam remote exec loki velli instance ni delete cheyalenu kaabatti manam local tesukunnam

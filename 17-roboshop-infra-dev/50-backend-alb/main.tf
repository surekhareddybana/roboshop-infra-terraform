module "backend_alb" {
  ...
  internal = "true" #if u define true lb is private lb
  version = "9.16.0"

  name    = "${var.project}-${var.environment}-backend-alb" #roboshop-backend-alb
  vpc_id  = local.vpc_id
...
resource "aws_lb_listener" "backend_alb" {
  ...
  default_action {
    ...
    fixed_response {
        ...
        message_body = "<h1>HELLO i am from backend_alb</h1>"
        status_code = "200"
    }
